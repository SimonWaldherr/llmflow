...
func (s *Server) normalizeWebRunOutput(cfg config.Config, fallbackRaw string) (config.Config, string) {
	outputPath := strings.TrimSpace(cfg.Output.Path)
	outputDir := filepath.Dir(outputPath)
	if outputDir == "" || outputDir == "." {
		outputDir, _ = s.allowedFileDir("output")
	}
	...
	cfg.Output.Path = filepath.Join(outputDir, base+".jsonl")
...
func (s *Server) handleDownloadFile(w http.ResponseWriter, r *http.Request) {
	...
	if format != "" {
		...
		if err := serveConvertedJSONLDownload(w, r, dst, name, format); err != nil {
...
func serveConvertedJSONLDownload(w http.ResponseWriter, r *http.Request, srcPath, srcName, format string) error {
	records, err := readJSONLRecords(srcPath)
	if err != nil {
		return err
	}

	base := strings.TrimSuffix(srcName, filepath.Ext(srcName))
	filename, contentType, payload, err := renderConvertedDownload(r.Context(), base, format, records)
	if err != nil {
		return err
	}
...
func readJSONLRecords(path string) ([]output.Record, error) {
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal(append([]byte(nil), line...), &record); err != nil {
			return nil, fmt.Errorf("decode jsonl line %d: %w", lineNo, err)
		}
		records = append(records, record)
	}
	if err := scanner.Err(); err != nil {
		return nil, fmt.Errorf("scan jsonl output: %w", err)
	}
	return records, nil
}

func renderConvertedDownload(ctx context.Context, baseName, format string, records []output.Record) (string, string, []byte, error) {
	switch format {
	case "", "jsonl":
		return baseName + ".jsonl", "application/x-ndjson", marshalJSONL(records), nil
	case "json":
...
func marshalJSONL(records []output.Record) []byte {
...
func renderStructuredDownload(ctx context.Context, baseName, format string, records []output.Record) (string, string, []byte, error) {
	tmpDir, err := os.MkdirTemp("", "llmflow-export-*")
	if err != nil {
		return "", "", nil, fmt.Errorf("create export temp dir: %w", err)
	}
	defer os.RemoveAll(tmpDir)

	cfg := config.OutputConfig{Type: format, Path: filepath.Join(tmpDir, baseName+downloadExtension(format))}
	contentType := downloadContentType(format)
	if format == "tsv" {
		cfg.Type = "csv"
		cfg.CSV.Delimiter = "\t"
	} else if format == "csv" {
	...
	if err != nil {
		return "", "", nil, fmt.Errorf("create %s export: %w", format, err)
	}
	writeErr := writer.WriteAll(ctx, records)
	closeErr := writer.Close()
	if writeErr != nil {
		return "", "", nil, fmt.Errorf("write %s export: %w", format, writeErr)
	}
	if closeErr != nil {
		return "", "", nil, fmt.Errorf("close %s export: %w", format, closeErr)
	}

	payload, err := os.ReadFile(cfg.Path)
	if err != nil {
		return "", "", nil, fmt.Errorf("read %s export: %w", format, err)
	}
	return baseName + downloadExtension(format), contentType, payload, nil
}
